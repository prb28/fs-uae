...
void branch_stack_pop_rts(uaecptr oldpc)
{
	...
	if (found) {
		...
		if (break_stack_pop && ((break_stack_s && regs.s) || (!break_stack_s && !regs.s))) {
			break_stack_push = false;
			break_stack_pop = false;
			debugmem_break(0);
		}
...
void branch_stack_push(uaecptr oldpc, uaecptr newpc)
{
	...
	if (regs.s) {
		if (stackframecntsuper < MAX_STACKFRAMES) {
			stackframecntsuper++;
		} else {
			stackframecntsuper = 0;
		}
	} else {
		if (stackframecnt < MAX_STACKFRAMES-10) {
			stackframecnt++;
		} else {
			stackframecnt = stackframecnt;
		}
		stackframecntsuper = 0;
	}
...
void debugmem_addsegs(TrapContext *ctx, uaecptr seg, uaecptr name, uae_u32 lock, bool residentonly)
{
	...
	if (name) {
		...
		if (!lock) {
			...
			get_native_path(lock, nativepath);
		}
